...
`KHULNASOFT_PRIVATE_KEY`, `KHULNASOFT_PUBLIC_KEY`, `KHULNASOFT_ROOT_CA`(if using self-signed certs), `OFFLINE_CC_MTLS_ENABLE` in [003_scanner_configmap.yaml](./003_scanner_configmap.yaml) and uncomment khulnasoft-grpc-scanner certs vloumemount and volumes block from [004_scanner_deploy.yaml](./004_scanner_deploy.yaml) file before applying the kubectl commands.
